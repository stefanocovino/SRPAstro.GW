...
#
#if !(-e $fc) then
#    set cmd = "SRPFindingChart -v -f '$argv[1]' -s 120 -c 1000 10000"
#    echo $cmd
#    eval $cmd
...
foreach f (`seq 1 1 $#argv`)
    @ n = $n + 1
    if !(-e $rt$n.tab) then
        set cmd = "SRPGWImportCats -v -i $rt$n.cat -o $rt$n.tab -f $argv[$f] -I Mrcn"
        echo $cmd
        eval $cmd
    endif
end
...
foreach f (`seq 1 1 $#argv`)
    @ n = $n + 1
    if !(-e $rt$n.temp) then
        set cmd = "SRPGWStats -f $rt$n:r_2.tab -c flux"
        echo $cmd
        set res = `$cmd`
...
foreach f (`seq 1 1 $#argv`)
    ...
    if !(-e $rt$n:r_4.tab) then
        set cmd = "SRPGWCalc -i $rt$n:r_3.tab -o $rt$n:r_4.tab -c ':peaky = peak / npix '"
        echo $cmd
        eval $cmd
...
# Command line
set n = 0
set strl = "./02.Marcon.Analisi.csh "
foreach f (`seq 1 1 $#argv`)
    @ n = $n + 1
